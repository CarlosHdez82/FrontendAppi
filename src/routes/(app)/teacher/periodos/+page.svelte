...
<script lang="ts">
    ...
    async function cargarPeriodos() {
        ...
        try {
            ...
            console.error("Error CUL Periods"); 
        } finally { 
            cargando = false; 
        }
    }
    ...
    async function guardar() {
        const url = editando 
            ? `https://gestion-de-horarios-final.onrender.com/update_period/${idSeleccionado}` 
            : "https://gestion-de-horarios-final.onrender.com/create_period";
...
{#if cargando}
    <LoadingSpinner />
{:else}
    <!-- Pasamos 'lista' a la prop data para habilitar búsqueda y conteo -->
