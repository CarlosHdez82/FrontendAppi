<script lang="ts">
    // ============================================================
    // admi/usuarios/+page.svelte — Gestión de Usuarios
    // ============================================================
    // CRUD completo de usuarios del sistema para el Administrador.
    // Al editar, el campo contraseña se oculta por seguridad
    // (usar /perfil para cambiarla).
    //
    // La función cargar() maneja múltiples estructuras posibles
    // de respuesta de la API para mayor robustez.
    // {#key usuarios} fuerza a Svelte a redibujar la tabla
    // cuando el array cambia, evitando problemas de reactividad.
    // ============================================================
    ...
    let formulario = $state({
        ...
        password_hash: "", role_id: 3, is_active: true
    });
    ...
    function prepararEdicion(u: any) {
        editando = true;
        // La API puede retornar 'user_id' o 'id' según el endpoint
        idSeleccionado = u.user_id || u.id;
        formulario = {
            first_name: u.first_name, last_name: u.last_name,
            email: u.email,
            password_hash: "", // Se vacía por seguridad al editar
    ...
    async function cargar() {
        ...
        try {
            ...
            if (!res.ok) { console.error("Respuesta fallida:", res.status); return; }

            const data = await res.json();
            console.log("Datos recibidos de la API:", data); // Log de diagnóstico en F12

            // Maneja múltiples estructuras posibles de respuesta de la API
...
{#if cargando}
    <LoadingSpinner />
{:else}
    <!-- {#key usuarios} fuerza redibujado de la tabla cuando el array cambia -->
    {#key usuarios}
...
<FormModal id="modalPrincipal" title={editando ? "Editar Usuario" : "Nuevo Usuario"} isEdit={editando} onSave={guardar}>
    <div class="row g-3 text-start">
        ...
        <!-- Campo contraseña: solo visible al crear, oculto al editar por seguridad -->
        {#if !editando}
            <div class="col-12">
