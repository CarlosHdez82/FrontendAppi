...
{#if cargando}
    <LoadingSpinner />
{:else}
    <!-- CAMBIO AQUÍ: Pasamos el array 'docentes' a la prop 'data' -->
    <DataTable {headers} data={docentes}>
        <!-- CAMBIO AQUÍ: Definimos el rowTemplate para que el DataTable pueda filtrar los datos -->
        {#snippet rowTemplate(d)}
            <tr>
                <td class="ps-4">
                    <div class="fw-bold">{d.first_name} {d.last_name}</div>
                    <div class="small text-muted">{d.email}</div>
